...
def fit_map(map_id, bin_width=2.0, max_dist=40.0):
    ...
    binned = df.groupby('bin').agg(
        mean_score=('matching_score_pct', 'mean'),
        count=('matching_score_pct', 'count')
    ).reset_index()
    ...
    try:
        ...
        return {'sigma': sigma, 'A': A, 'B': B, 'R2': r2, 'n_bins': len(binned), 'n_vertices': len(df)}
    except Exception as e:
        return {'sigma': np.nan, 'A': np.nan, 'B': np.nan, 'R2': np.nan, 
                'n_bins': len(binned), 'n_vertices': len(df), 'error': str(e)}
...
print("="*80)
mf = rdf[rdf['is_multifocal'] == True]
for _, row in mf.iterrows():
    print(f"  Map {row['map_id']:3d} | {row['region']:8s} | ref={row['ref_catheters']} | "
...
for ref in [1, 2]:
    ...
    for region in regions:
        ...
        focal_maps = rdf[mask_base & (rdf['is_multifocal'] == False)]
        n_all = len(all_maps)
        ...
        print(f"  {region:8s} | n={n_all}->{n_focal} (excl {n_excluded}) | "
...
print("="*80)
mf_r2 = rdf[rdf['is_multifocal'] == True]['R2']
focal_r2 = rdf[rdf['is_multifocal'] == False]['R2']
print(f"  Multifocal (n={len(mf_r2)}): R2 median={mf_r2.median():.3f} [{mf_r2.min():.3f}; {mf_r2.max():.3f}]")
print(f"  Focal      (n={len(focal_r2)}): R2 median={focal_r2.median():.3f} [{focal_r2.min():.3f}; {focal_r2.max():.3f}]")
...
for region in regions:
    ...
    sc_focal = rdf[(rdf['region'] == region) & (rdf['ref_catheters'] == 1) & (rdf['is_multifocal'] == False)]['sigma'].dropna()
    dc_focal = rdf[(rdf['region'] == region) & (rdf['ref_catheters'] == 2) & (rdf['is_multifocal'] == False)]['sigma'].dropna()
    if len(sc_all) >= 2 and len(dc_all) >= 2:
        ...
        print(f"  {region:8s} | ALL: U={stat_all:.0f}, p={p_all:.4f} (n={len(sc_all)}v{len(dc_all)}) | "
