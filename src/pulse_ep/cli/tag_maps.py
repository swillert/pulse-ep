...
ap.add_argument('--skip-existing', action='store_true', help='Skip maps with attributes already set')
...
def _get_xyz_from_points(session, map_id) -> np.ndarray | None:
    ...
    points = (session.query(EPMapPoint.position_x, EPMapPoint.position_y, EPMapPoint.position_z)
...
def name_detect_atrium(name: str, study: str) -> str | None:
    t = (name + ' ' + study).upper()
    if re.search(r'\bVT\b',                             t): return 'LV'
    if re.search(r'\bLA\b|LEFT[\s_-]*ATRI|RE[-]?LA\b',  t): return 'LA'
    if re.search(r'\bRA\b|RIGHT[\s_-]*ATRI|RE[-]?RA\b', t): return 'RA'
    # "Vera" sub-regions are always the left atrium in AF ablation context
    if re.search(r'\bVERA\b',                           t): return 'LA'
    return None
...
_TYPE_PATTERNS = [
    ...
    (r'\bICPM\b|IC[\s_-]*PM\b|IC[\s_-]*PACE|IC[\s_-]*PATTERN|PATTERN[\s_-]*MATCH'
     r'|PACE[\s_-]*MAP\b|PACE[\s_-]*PATTERN\b|PACEMAP\b|PACEPM',   'pacemapping'),
...
def _centroid(raw, ncols=3) -> np.ndarray | None:
    ...
    if arr.ndim == 1: arr = arr.reshape(-1, ncols)
...
def coord_detect_part(map_xyz, reference_centroid) -> str | None:
    ...
    """
    c = _centroid(map_xyz)
    if c is None or reference_centroid is None: return None
...
def build_attributes(m, vera_alles_centroid, xyz=None) -> dict:
    ...
    attrs = {}
    if atrium:           attrs['atrium']  = atrium
    if final_part:       attrs['part']    = final_part
    if final_type:       attrs['type']    = final_type
    if final_pacemap is True:  attrs['pacemap'] = True
    if final_pacemap is False: attrs['pacemap'] = False
...
def main():
    ...
    with get_db_session() as session:
        maps = session.query(EPMapModel).order_by(
            EPMapModel.study_name, EPMapModel.map_name).all()
