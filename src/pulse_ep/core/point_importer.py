...
def _find_on_annotation_by_filename(study_dir: str, map_name: str, start_time: int) -> dict:
    """
    ...
    for idx, pt_elem in enumerate(point_elements):
        ...
        try:
            ...
            if pd["start_time"] is not None:
                fallback = _find_on_annotation_by_filename(study_dir, map_name, pd["start_time"])
                for cat, fpath in fallback.items():
                    ...
                    if coords is not None:
                        ...
                        for conn_name, c in CONNECTOR_SEARCH_NAMES:  # noqa: B007
