...
def render_panel(session, panel_info, output_png):
    ...
    print(f"  Distance mask ({DISTANCE_THRESHOLD} mm): {n_masked}/{len(vertices)} "
...
def compose_figure(panel_pngs):
    ...
    fig.patches.append(FancyBboxPatch(
        (0.02, 0.945), 0.96, 0.012, transform=fig.transFigure,
        facecolor=RED, edgecolor='none', clip_on=False,
        boxstyle='square,pad=0'))

    fig.text(0.025, 0.915,
             'Wo der Algorithmus scharf sieht — und wo nicht',
             fontsize=12, fontweight='bold', color=GRAY_DARK, va='top')
    fig.text(0.025, 0.87,
             'Matching-Score-Verteilung auf der Vorhofoberfläche. '
             'Kreuz = Vertex mit höchstem Score.',
             fontsize=8, color=GRAY_MID, va='top', fontstyle='italic')

    gs = fig.add_gridspec(1, 4, width_ratios=[1, 1, 1, 0.05],
                          left=0.03, right=0.97, bottom=0.05, top=0.81,
                          wspace=0.08)

    for i, (panel, png_path) in enumerate(zip(PANELS, panel_pngs)):
        ...
        ax.text(-0.06, 1.08, panel['letter'], transform=ax.transAxes,
                fontsize=16, fontweight='bold', color=GRAY_DARK, va='top')
        ax.text(0.04, 1.07, panel['title'], transform=ax.transAxes,
                fontsize=9.5, fontweight='bold', color=GRAY_DARK, va='top')
        ax.text(0.04, 0.975, panel['subtitle'], transform=ax.transAxes,
                fontsize=7.5, color=GRAY_MID, va='top', fontstyle='italic')

        box_text = f'σ = {panel["sigma"]} mm\n$A_{{≥θ}}$ = {panel["area"]} cm²'
        ax.text(0.97, 0.04, box_text, transform=ax.transAxes,
                fontsize=6.5, color='#D5D8DC', va='bottom', ha='right',
    ...
    sm = plt.cm.ScalarMappable(cmap=econ_cmap,
