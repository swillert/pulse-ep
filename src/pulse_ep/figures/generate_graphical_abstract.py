...
OUTDIR      = os.path.join(SCRIPT_DIR, '..', 'thesis-med', '_paper',
                           'clinical-paper', 'figs')
...
parser = argparse.ArgumentParser(
    description='Generate graphical abstract for clinical paper.')
parser.add_argument('--style', choices=['journal', 'economist'],
                    default='journal',
...
if STYLE == 'economist':
    ...
    plt.rcParams.update({
        ...
        'font.sans-serif': ['Source Sans 3', 'Source Sans Pro',
        ...
        'figure.facecolor': BG, 'figure.dpi': 300,
...
cbar_cmap = LinearSegmentedColormap.from_list('heat', [
    (0.00, '#1C2833'),
    (0.30, '#1A5276'),
    (0.50, '#00857C'),
    (0.68, '#F4D03F'),
...
def render_panel(session, panel_info, output_png):
    ...
    point_rows = (
        ...
        .filter(EPMapPoint.map_id == map_id, EPMapPoint.position_x != None)
        .all()
    )
    point_xyz = np.array([[r[0], r[1], r[2]] for r in point_rows])
    print(f"  Map {map_id}: {len(vertices)} vertices, {len(triangles)} faces, "
    ...
    plotter.add_mesh(pv_mesh, scalars='act', cmap=cmap, clim=clim,
                     show_scalar_bar=False, nan_color='#BDBDBD', nan_opacity=0.4)
    ...
    if valid_mask.any():
        ...
        plotter.add_mesh(origin_mesh, color='white', point_size=18,
...
def fit_map(map_id, bin_width=2):
    ...
    try:
        ...
        popt, _ = curve_fit(gauss, d_centers, scores,
...
def load_boxplot_data():
    ...
    ra_lat = xls[(xls['part'].str.contains('lateral', case=False, na=False)) &
                 (xls['pacemap'] == True)].copy()
    ...
    print(f"  SC (n={len(sigma_sc)}): median {np.median(sigma_sc):.1f} mm"
          if sigma_sc else "  SC: no data")
    print(f"  DC (n={len(sigma_dc)}): median {np.median(sigma_dc):.1f} mm"
          if sigma_dc else "  DC: no data")
...
def compose(panel_pngs, sigma_sc, sigma_dc):
    ...
    FIGS_DIR = os.path.join(SCRIPT_DIR, '..', 'thesis-med', '_paper',
                            'clinical-paper', 'figs')
    if STYLE == 'economist':
    ...
    gs = fig.add_gridspec(
        ...
        height_ratios=[1, 1],
        left=0.03, right=0.97, bottom=0.14, top=0.82,
    ...
    if STYLE == 'economist':
        fig.patches.append(FancyBboxPatch(
            (0.02, 0.985), 0.96, 0.012, transform=fig.transFigure,
            facecolor=ECON_RED, edgecolor='none', clip_on=False,
            boxstyle='square,pad=0'))

    # ── Title ──────────────────────────────────────────────
    title_y = 0.96 if STYLE == 'economist' else 0.98
    fig.text(0.50, title_y,
             'How Precise Is Atrial Pace Mapping '
    ...
    fig.text(0.50, subtitle_y,
             'Double-chamber reference reduces σ from up to 25 mm to 5 mm '
             'across all atrial regions',
             fontsize=9 if STYLE == 'economist' else 8.5,
             color=SUBTITLE_COLOR, ha='center', va='top',
             fontstyle='italic')
    ...
    sc_row_mid = sc_schema_pos.y0 + sc_schema_pos.height / 2
    dc_row_mid = dc_schema_pos.y0 + dc_schema_pos.height / 2

    fig.text(sc_schema_pos.x0 + sc_schema_pos.width / 2,
             sc_schema_pos.y1 + 0.015,
             PANELS[0]['label'], fontsize=8, fontweight='bold',
             color=AMBER,
             ha='center', va='bottom', transform=fig.transFigure)
    fig.text(dc_schema_pos.x0 + dc_schema_pos.width / 2,
             dc_schema_pos.y1 + 0.015,
             PANELS[1]['label'], fontsize=8, fontweight='bold',
             color=SLATE,
             ha='center', va='bottom', transform=fig.transFigure)

    # ── Catheter schematics (left column) ──────────────────
    for ax, schema_path in [(ax_sc_schema, sc_schema_path),
                             (ax_dc_schema, dc_schema_path)]:
    ...
    for (schema_ax, heat_ax), acol in zip(
            [(ax_sc_schema, ax_sc_heat), (ax_dc_schema, ax_dc_heat)],
            arrow_colors):
    ...
    for ax, png_path, panel in zip([ax_sc_heat, ax_dc_heat],
                                    panel_pngs, PANELS):
        ...
        ax.text(0.97, 0.04, box_text, transform=ax.transAxes,
                fontsize=7, color='#D5D8DC', va='bottom', ha='right',
                fontfamily='monospace',
                bbox=dict(boxstyle='round,pad=0.3', facecolor=PANEL_BG,
                          edgecolor='#2C3E50', linewidth=0.5, alpha=0.88))

    # ── Annotation arrows on heatmaps ────────────────────────
    # SC: large diffuse region — arrow tip ends at edge of colored area
    ax_sc_heat.annotate(
        'diffuse',
        xy=(0.42, 0.50), xycoords='axes fraction',       # edge of colored region
        xytext=(0.12, 0.88), textcoords='axes fraction',  # label top-left
    ...
    ax_dc_heat.annotate(
        'focal',
        xy=(0.45, 0.52), xycoords='axes fraction',       # edge of colored region
        xytext=(0.12, 0.88), textcoords='axes fraction',  # label top-left
        fontsize=7, color='white', fontweight='bold',
        ha='center', va='center',
        arrowprops=dict(arrowstyle='-|>',
    ...
    for heat_ax in [ax_sc_heat, ax_dc_heat]:
        cax = inset_axes(heat_ax,
                         width="5%", height="45%",
                         loc='lower left',
                         borderpad=1.2)
        cax.set_facecolor('none')
        cax.imshow(gradient, aspect='auto', cmap=cbar_cmap,
                   origin='lower', extent=[0, 1, 50, 100])
        cax.set_xlim(0, 1)
        cax.set_ylim(50, 100)
        cax.set_xticks([])
        cax.set_yticks([50, 75, 100])
        cax.tick_params(labelsize=4.5, colors='#D5D8DC', pad=1, length=2,
    ...
    if sigma_sc and sigma_dc:
        ...
        bp = ax_box.boxplot(bp_data, positions=[1, 2], widths=0.5,
        ...
        for i, (data, color) in enumerate(zip(bp_data, box_colors)):
            jitter = np.random.default_rng(42).normal(0, 0.06, len(data))
            ax_box.scatter([i + 1] * len(data) + jitter, data,
                           s=16, c=color, alpha=0.6, zorder=5,
                           edgecolors='none')
        ...
        ax_box.text(0.5, 0.5, 'No data', transform=ax_box.transAxes,
    ...
    fig.text(0.50, 0.06,
             ...
             bbox=dict(boxstyle='round,pad=0.5', facecolor=BOX_BG,
                       edgecolor=GRAY_MID, linewidth=1.0))

    # ── Economist: source line ─────────────────────────────
    if STYLE == 'economist':
        fig.text(0.03, 0.005,
                 'Source: own data, CARTO-3 system · UKSH Kiel',
                 fontsize=6, color=GRAY_MID, va='bottom',
                 fontfamily='monospace')
