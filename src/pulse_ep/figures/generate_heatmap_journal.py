...
OUTPATH    = os.path.join(SCRIPT_DIR, '..', 'thesis-med', '_paper',
                          'method-paper', 'figs', 'fig_heatmap.pdf')
...
def compose_figure(panel_pngs):
    ...
    gs = fig.add_gridspec(1, 4, width_ratios=[1, 1, 1, 0.05],
                          left=0.03, right=0.97, bottom=0.05, top=0.88,
                          wspace=0.08)
    ...
    for i, (panel, png_path) in enumerate(zip(PANELS, panel_pngs)):
        ...
        ax.text(-0.06, 1.10, panel['letter'], transform=ax.transAxes,
                fontsize=14, fontweight='bold', color=GRAY_DARK, va='top')
        ax.text(0.06, 1.08, panel['title'], transform=ax.transAxes,
                fontsize=9, fontweight='bold', color=GRAY_DARK, va='top')
    ...
    sm = plt.cm.ScalarMappable(cmap=cbar_cmap,
