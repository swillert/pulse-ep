...
def map_as_openep(map_id):
    ...
    with get_db_session() as session:
        ...
        try:
            scale = request.args.get("signal_scale_to_mv")
            ids = request.args.get("ablation_ids", "")
            ecg = request.args.get("ecg_channels")
            userdata = stored_userdata(
                session,
                epmap_model,
                include_points=include_points,
                include_signals=request.args.get("include_signals", "0").lower()
                in {"1", "true", "yes"},
                ablation_scope=request.args.get("ablation_scope", "none"),
                ablation_ids=[int(x) for x in ids.split(",")] if ids else None,
                signal_scale_to_mv=float(scale) if scale is not None else None,
                ecg_channels=ecg.split(",") if ecg is not None else None,
            )
        except ValueError as exc:
            return jsonify({"error": str(exc)}), 400
    return jsonify(_jsonable(userdata))
...
def get_epmaps_by_ids():
    ...
    with get_db_session() as session:
        ...
        epmaps = (
            ...
            .filter(EPMapModel.id.in_(epmap_ids))
            .all()
        )
...
def get_mesh_data():
        ...
        with get_db_session() as session:
            ...
            try:
                data = export_map(
                    session,
                    model,
                    request.args.get("scalar_name") or None,
    ...
    try:
        ...
        if (
            vertices is None
            or faces is None
            or scalar_data is None
            or normalized_scalar_data is None
        ):
    ...
    try:
        ...
        point_data = {
            ...
            "normalized_scalar_data": normalized_point_scalar_data.tolist()
            if normalized_point_scalar_data is not None
            else None,
        }
    ...
    return app.response_class(response=json_raw, status=200, mimetype="application/json")
...
def filter_epmaps_by_attributes():
    ...
    with get_db_session() as session:
        ...
        query = query.filter(
            EPMapAttributes.attributes.op("@>")(flask_json.dumps(converted_filters))
        )
...
def set_attributes_for_epmaps():
    ...
    with get_db_session() as session:
        ...
        try:
            ...
            print(
                f"Re-querying after commit for map_id {test_id}: {test_entry.attributes}"
            )  # Check if "a" is set to "sad"

        except Exception as e:
            print("Error during session commit:", e)
            session.rollback()
            return jsonify(
                {"msg": "Failed to commit changes to the database.", "error": str(e)}
            ), 500
    ...
    try:
        with get_db_session() as session:
            ...
            metadata_entries = session.query(
                AttributeMetadata.name, AttributeMetadata.data_type, AttributeMetadata.default_value
            ).all()
            distinct_attributes = [
                {"name": entry[0], "type": entry[1], "default_value": entry[1]}
...
def calculate_areas_for_intervals_endpoint():
    ...
    try:
        areas = ep_map.calculate_areas_for_intervals(
            intervals, scalar_name=scalar_name, distance=distance
        )
    except TypeError:
...
def save_report():
    ...
    with get_db_session() as session:
        try:
            ...
            new_report = ReportModel(map_ids=map_ids, additional_data=additional_data)
    ...
    try:
        ...
        with get_db_session() as session:
            ...
            if colormap_id:
                ...
                if cm and cm.intervals and len(cm.intervals) >= 2:
                    ...
                    interval_labels = [
                        f"{ivs[i]:.1f}–{ivs[i + 1]:.1f}" for i in range(len(ivs) - 1)
                    ]
            ...
            attr_keys = list(dict.fromkeys(k for mid in map_ids for k in attr_data.get(mid, {})))
        ...
        for mid in map_ids:
            ...
            try:
                ...
                rel_pairs = [
                    (lo / 100.0 * float(max_s), hi / 100.0 * float(max_s))
                ...
                for label, a_abs, a_rel in zip(interval_labels, area_values, rel_area_values):  # noqa: B905
                    row[f"{label} (abs)"] = (
                        round(float(a_abs), 4)
                        if a_abs is not None and not np.isnan(a_abs)
                        else None
                    )
                    row[f"{label} (rel)"] = (
                        round(float(a_rel), 4)
                        if a_rel is not None and not np.isnan(a_rel)
                        else None
                    )
                for ak in attr_keys:
        ...
        headers = (
            ["ID", "Study", "Map Name", "Total Area (cm²)", "Min", "Max", "Mean", "Std"]
            + paired_labels
            + attr_keys
        )
